...
function initSistemaPosLogin() {
    ...
    // A chamada para atualizar os indicadores foi removida daqui para evitar erro de timing.
}
...
function handleNavClick(event, targetView) {
    ...
    switch (targetView) {
        case 'dashboard':
            // Este é o local correto para atualizar tudo relacionado ao dashboard.
            listarHistorico();
            listarCTCsSalvas();
            atualizarIndicadoresDashboard(); // CHAMADA CENTRALIZADA AQUI
            break;
        case 'simulacao':
...
function atualizarIndicadoresDashboard() {
    ...
    // Adicionada verificação de segurança para garantir que o canvas existe antes de continuar
    const canvas = document.getElementById('graficoTiposBeneficio');
    if (!canvas) {
        return; // Sai da função se a tela do dashboard não estiver ativa.
    }

...
function adicionarLinhaPeriodoCTC(i = '', f = '', d = '0', fo = '') {
    ...
    details.innerHTML = `
      <summary>
        <span style="font-weight:600;">Período #${idx}</span>
        <span class="range-text">${i ? formatarDataBR(i, false) : 'Início'} — ${f ? formatarDataBR(f, false) : 'Fim'}</span>
        <span class="stats">(Bruto: <span class="bruto-sum">0</span>d / Líq: <span class="liq-sum">0</span>d)</span>
      </summary>
      <div class="periodo-body">
        <div class="form-grid" style="grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:8px;">
          ...
          <label>Fonte / Observação:<input type="text" class="ctc-fonte" value="${fo}"></label>
        </div>
        <div style="text-align:right;margin-top:8px;">
          <button class="danger" onclick="removerLinhaPeriodoCTC(this)">Remover</button>
        </div>
      </div>
    `;
...
function removerLinhaPeriodoCTC(btn) {
    const details = btn.closest('.ctc-periodo');
...
function calcularTempoPeriodosCTC() {
    ...
    items.forEach((det, index) => {
        const iE = det.querySelector('.ctc-inicio'),
              ...
              sumBr = det.querySelector('.bruto-sum'),
              sumLi = det.querySelector('.liq-sum'),
              range = det.querySelector('.range-text');

        if (iE && fE && iE.value && fE.value) {
            const i = new Date(iE.value + 'T00:00:00'),
                  f = new Date(fE.value + 'T00:00:00');
            if (f >= i) {
                const dias = Math.ceil((f - i) / 86400000) + 1;
                bE.value = dias;
                const ded = parseInt(dE.value) || 0;
                lE.value = Math.max(0, dias - ded);
                sumBr.textContent = dias;
                sumLi.textContent = Math.max(0, dias - ded);
                range.textContent = `${formatarDataBR(iE.value, false)} — ${formatarDataBR(fE.value, false)}`;
            }
        }
    });
    calcularTempoTotalCTC();
